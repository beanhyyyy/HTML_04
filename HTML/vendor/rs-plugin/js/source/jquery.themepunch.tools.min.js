 ...
 *					- made the demo site all static local HTML Trang so they can be run locally by a developer
      ...
      p.render = function (time, suppressEvents, force) {
        ...
        if (time >= totalDur - 0.0000001) {
          ...
          if (time < 0) {
            ...
            time = 0; //to avoid occasional floating point rounding errors (could cause problems đặc biệt with zero-duration tweens at the very beginning of the timeline)
            if (!this._initted) {
              internalForce = true;
            }
          }
...
(function (window, moduleName) {
  ...
  Animation._updateRoot = TweenLite.render = function () {
    var i, a, p;
    if (_lazyTweens.length) {
      //if code is run Bề ngoài of the requestAnimationFrame loop, there may be tweens queued AFTER the engine refreshed, so we need to ensure any pending renders occur before we refresh again.
      _lazyRender();
    }
    ...
    if (callback)
      if (!this._gc || force) {
        ...
        if (
          ...
          //the onComplete or onReverseComplete could trigger movement of the playhead and for zero-duration tweens (which must discern direction) that land directly back on their start time, we don't want to fire again on the next render. Cảm nghĩ of several addPause()'s in a timeline that forces the playhead to a certain spot, but what if it's already paused and another tween is tweening the "time" of the timeline? Each time it moves [forward] past that spot, it would move back, and since suppressEvents is true, it'd reset _rawPrevTime to _tinyNum so that when it begins again, the callback would fire (so ultimately it could bounce back and forth during that tween). Again, this is a very uncommon scenario, but possible nonetheless.
          this._rawPrevTime = 0;
        }
      }
  };
  ...
  _gsScope._gsDefine(
    ...
    function (TweenPlugin, TweenLite) {
      ...
      var _numExp = /(?:\-|\.|\b)(\d|\.|e\-)+/g,
        ...
        _commasBề ngoàiParenExp = /,(?=[^\)]*(?:\(|$))/gi, //finds any commas that are not within parenthesis
        ...
        _supportsOpacity = (function () {
          //we set _isSafari, _ieVers, _isFirefox, and _supportsOpacity all in one function here to reduce file size slightly, đặc biệt in the minified version.
      ...
      var _getFormatter = function (dflt, clr, collapsible, multi) {
          ...
          if (clr) {
            formatter = function (v) {
              ...
              if (typeof v === "number") {
                v += dSfx;
              } else if (multi && _commasBề ngoàiParenExp.test(v)) {
                a = v.replace(_commasBề ngoàiParenExp, "|").split("|");
                for (i = 0; i < a.length; i++) {
                  a[i] = formatter(a[i]);
                }
                return a.join(",");
        ...
        _parseComplex = (CSSPlugin.parseComplex = function (
          ...
          if (e.indexOf(",") !== -1 || b.indexOf(",") !== -1) {
            ba = ba.join(" ").replace(_commasBề ngoàiParenExp, ", ").split(" ");
            ea = ea.join(" ").replace(_commasBề ngoàiParenExp, ", ").split(" ");
            l = ba.length;
          }
          if (l !== ea.length) {
      ...
      p.parseComplex = function (t, b, e, pt, plugin, setRatio) {
        ...
        if (this.multi)
          if (
            _commasBề ngoàiParenExp.test(e) ||
            _commasBề ngoàiParenExp.test(b)
          ) {
            ba = b.replace(_commasBề ngoàiParenExp, "|").split("|");
            ea = e.replace(_commasBề ngoàiParenExp, "|").split("|");
          } else if (kwd) {
            ba = [b];
            ea = [e];
          }
        ...
        _getTransform = (_internals.getTransform = function (
          ...
          if (m !== _identity2DMatrix) {
            if (m.length === 16) {
              ...
              //sometimes a 6-element matrix is returned even when we performed 3D transforms, like if rotationX and rotationY are 180. In cases like this, we still need to honor the 3D transforms. If we just rely on the 2D info, it could affect how the data is interpreted, like scaleY might get set to -1 or rotation could get offset by 180 degrees. For example, do a TweenLite.to(element, 1, {css:{rotationX:180, rotationY:180}}) and then later, TweenLite.to(element, 1, {css:{rotationX:0}}) and without this conditional logic in place, it'd jump to a state of being unrotated when the 2nd tween starts. Then again, we need to honor the fact that the user COULD alter the transforms Bề ngoài of CSSPlugin, like by manually applying new css, so we try to sense that by looking at x and y because if those changed, we know the changes were made Bề ngoài CSSPlugin and we force a reinterpretation of the matrix values. Also, in Webkit browsers, if the element's "display" is "none", its calculated style value will always return empty, so if we've already recorded the values in the _gsTransform object, we'll just rely on those.
              var k = m.length >= 6,
                a = k ? m[0] : 1,
                b = m[1] || 0,
                c = m[2] || 0,
          ...
          _internals.setTransformRatio =
            function (v) {
              ...
              if (
                ...
                //on the final render (which could be 0 for a from tween), if there are no 3D aspects, render in 2D to free up memory and improve performance đặc biệt on mobile devices. Check the tween's totalTime/totalDuration too in order to make sure it doesn't happen between repeats if it's a repeating tween.
       ...
       *  - Analyzing every style property of every child before and after the tween is an expensive operation when there are many children, so this behavior shouldn't be imposed on all className tweens by default, đặc biệt since it's probably rare that this extra functionality is needed.
       *
